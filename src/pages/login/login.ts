...
export class LoginPage {
  ...
  constructor(public navCtrl: NavController, private fb: Facebook, private googlePlus: GooglePlus, public loadingCtrl: LoadingController, public http: HttpClient, public storage: Storage, private linkedin: LinkedIn, public toast: Toast) {}
  ...
  fbLogin() {
    ...
    this.fb.login(['public_profile', 'email']).then((res: FacebookLoginResponse) => {
      load.present();
      this.fb.api('/' + res.authResponse.userID + '?fields=id,name,email,picture', []).then((data) => {
        let newstring = data.name.split(/[ ,]+/);
        let [fname, lname] = newstring;
        this.http.post('http://sprypixels.com/demo/askinsurpedia/public/api/fb', {
          fname: fname,
          lname: lname,
          email: data.email,
          fbid: data.id,
  ...
  gLogin() {
    ...
    this.googlePlus.login({}).then((res) => {
      load.present();
      this.http.post('http://sprypixels.com/demo/askinsurpedia/public/api/google', {
        fname: res.givenName,
  ...
  sendMail(email) {
    this.http.post('http://sprypixels.com/demo/askinsurpedia/public/api/mail/later', {
      email: email
    }).subscribe((data) => {
